void standby() {
  ...
  uint16 Vbat = analogRead(batt);//adcSensor[2];
  float Vbat_f = ((float(Vbat) * 3.30 / 4095.00) / (22.00 * 12.60 / 172.00)) * 12.60 + 0.14;
  dtostrf(Vbat_f, 4, 1, buff);
  Vbat_f = Vbat_f * 10.00;
  uint8 Vbat_i = (int) Vbat_f;
  lcd.drawString(11, 0, buff);
  lcd.drawString(15, 0, "v");
  uint8 Bar = map(Vbat_i, 111, 124, 0, 5);
  if (Vbat_i < 112) {
    led_OFF;
    lcd.drawString(9, 0, "X");
    if (Vbat_i < 111) Bar = 0;
  }
  else if (Vbat_i <= 115) {
    led_ON;
...
void menu() {
  ...
  while (1) {
    ...
    switch (index) {
      case 0:
        sprintf(buff, "PATH PLANNING:%2d", ee.PLAN + 1);
...
void planning_1() {
  ...
  while (1) {
    ...
    switch (index) {
      case 0:
        sprintf(buff, " PLAN:%2d - I:%2d  ", ee.PLAN + 1, ee.I);
      ...
      case 1:
        ...
        if (!PLUS) {
          while (!PLUS) {}
          if (++ee.ACTION[ee.PLAN][ee.I] > 8) ee.ACTION[ee.PLAN][ee.I] = 0;
          action_list();
        }
        if (!MIN) {
          while (!MIN) {}
          if (--ee.ACTION[ee.PLAN][ee.I] < 0) ee.ACTION[ee.PLAN][ee.I] = 8;
          action_list();
        }
        break;
      case 2:
  ...
  while (1) {
    ...
    switch (index) {
      case 0:
        ...
        choose(0); jump_menu();
        sprintf(buff, "P%02d", ee.jumpP[ee.PLAN][ee.I] + 1);
...
void copyPlan() {
  ...
  while (1) {
    ...
    switch (index) {
      case 0:
        choose(1); sprintf(buff, "%1d", PSUB[0] + 1); lcd.draw1x2String(7, 0, buff);
        choose(0); sprintf(buff, "%1d", PSUB[1] + 1); lcd.draw1x2String(14, 0, buff);
        ...
        break;
      case 1:
        choose(0); sprintf(buff, "%1d", PSUB[0] + 1); lcd.draw1x2String(7, 0, buff);
        choose(1); sprintf(buff, "%1d", PSUB[1] + 1); lcd.draw1x2String(14, 0, buff);
        choose(0); sprintf(buff, "%02d", ISUB[0]); lcd.draw1x2String(4, 2, buff);
        choose(0); sprintf(buff, "%02d", ISUB[1]); lcd.draw1x2String(11, 2, buff);
        choose(0); invers_menu(invers_mode);
        if (!PLUS) {
    ...
    if (!OK) {
      ...
      lcd.draw1x2String(0, 2, "    Success!    ");
      sprintf(buff, " Copied to P:%1d  ", PSUB[1] + 1);
      lcd.draw1x2String(0, 4, buff);
      delay(1000);
      lcd.clear();
    }
...
void clearPlan() {
  ...
  while (1) {
    ...
    switch (index) {
      case 0:
        choose(1); sprintf(buff, "%1d", PSUB + 1); lcd.draw1x2String(12, 1, buff);
        ...
        break;
      case 1:
        choose(0); sprintf(buff, "%1d", PSUB + 1); lcd.draw1x2String(12, 1, buff);
...
void invers_menu(int8 invers_mode) {
  switch (invers_mode) {
    case off: lcd.drawTile(4, 4, 1, uncheck); break;
    case on: lcd.drawTile(4, 4, 1, check); break;
  }
